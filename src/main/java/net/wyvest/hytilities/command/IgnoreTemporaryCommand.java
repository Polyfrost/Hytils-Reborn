...
public class IgnoreTemporaryCommand extends Command {
    ...
    private static final File file = new File(Hytilities.INSTANCE.modDir, "ignore.json");
    ...
    public void handle(@DisplayName("Player Name") String playerName, @DisplayName("Time") @Greedy String time) {
        Multithreading.runAsync(() -> {
            try {
                ...
                Hytilities.INSTANCE.sendMessage("Success! The user has been ignored, and will be removed from your ignore list in " + time);
            } catch (Exception e) {
                e.printStackTrace();
                Hytilities.INSTANCE.sendMessage("An error has occured and the user has not been ignored.");
            }
        });
    }
